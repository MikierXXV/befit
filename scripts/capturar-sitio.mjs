  ...
  for (const tema of TEMAS) {
    ...
    const huella = () => pagina.evaluate(() => {
      ...
      return [
        ...
        // Un menú que se despliega sí hace algo, aunque no cambie de ruta ni guarde nada.
        String(document.querySelectorAll('[aria-expanded="true"]').length),
      ].join('|');
    });
    /*
     * Los mandos que SE VEN. En el móvil, idioma y tema viven dentro del menú, escondidos hasta
     * abrirlo: pulsar uno escondido esperaba 30 s a que fuera visible y reventaba el recorrido. El
     * botón del menú sí se pulsa, y cuenta como que hace algo si despliega el menú.
     */
    const SELECTOR_MANDOS = '#cabecera [data-accion]:visible, #cabecera button:visible';
